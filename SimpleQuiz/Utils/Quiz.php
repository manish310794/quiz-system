...
class Quiz implements Base\IQuiz {
    ...
    {
        $quizobj = \ORM::for_table('quizzes')->join('categories', array('quizzes.category', '=', 'categories.id'))->select_many('quizzes.name', 'quizzes.description', 'quizzes.duration', array('category' => 'categories.name'), 'quizzes.active')->find_one($id);
        ...
        if ($quizobj) {
			...
			$this->_duration = $quizobj->duration;
	...
	*/
     ...
     * @return bool
     */
    public function addQuestion($text, $type, Array $answers, $url, $embedtype)
    {
        ...
        if ($newquestion->save())
        {
            ...
            $this->_question = new $questionType($newquestion->id(),$num, $this->_id, $text, $url, $embedtype);
     ...
     * @return bool
     */
    public function updateQuestion($questionid, $text, $url)
    {
        $this->getQuestion($questionid)->update($text);
		$this->getQuestion($questionid)->updateUrl($url);
	...
	public function getQuestion($questionid)
	{
		$question = \ORM::for_table('questions')->where('quiz_id', $this->_id)->where('num', $questionid)->find_one();
		return new RadioQuestion($question['id'], $question['num'], $this->_id, $question['text'], $question['url'], $question['type']);
	}
    ...
    public function populateQuestions()
    {
        ...
        foreach ($quizquestions as $question)
        {
             ...
             */
            $questionObject = new RadioQuestion($question['id'], $question['num'], $this->_id, $question['text'], $question['url'], $question['type']);

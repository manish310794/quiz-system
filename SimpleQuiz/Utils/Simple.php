...
class Simple implements Base\ISimple {
    ...
    public function addQuiz(Array $quizmeta)
    {
		...
		$quiz->duration = $quizmeta['duration'];
    ...
    public function getQuizzes($active = true) {
        ...
        if ($active) {
            $quizzes = \ORM::for_table('quizzes')->join('categories', array('quizzes.category', '=', 'categories.id'))->select_many('quizzes.id', 'quizzes.name', 'quizzes.description', 'quizzes.duration', array('category' => 'categories.name'), 'quizzes.active')->where('active',1)->find_many();
        } else {
            $quizzes = \ORM::for_table('quizzes')->join('categories', array('quizzes.category', '=', 'categories.id'))->select_many('quizzes.id', 'quizzes.name', 'quizzes.description', 'quizzes.duration', array('category' => 'categories.name'), 'quizzes.active')->find_many();
        }
        return $quizzes;
    }
